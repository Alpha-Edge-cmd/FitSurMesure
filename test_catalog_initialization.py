...
def run():
    with appmod.app.app_context():
        # --------------------------------------------------------------
        # 1) Import première fois : base vide -> 111 créés, tous pending
        # --------------------------------------------------------------
        assert count_existing_exercises() == 0
        rapport1 = init_main(argv=[])
        assert rapport1["total"] == 486
        assert rapport1["created"] == 486
        assert rapport1["updated"] == 0
        assert rapport1["pending"] == 486
        ...
        # --------------------------------------------------------------
        cible = "developpe_couche_barre_pecs"
        ...
        assert len(lignes) == Exercise.query.filter_by(review_status="pending").count() == 485
